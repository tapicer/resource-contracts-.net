...
namespace System.Compiler{
  ...
  public class Normalizer : StandardVisitor{
    ...
    public override DelegateNode VisitDelegateNode(DelegateNode delegateNode){
      ...
      if (invoke != null && endInvoke != null)
        invoke.ReturnAttributes = endInvoke.ReturnAttributes = this.ExtractAttributes(delegateNode.Attributes, AttributeTargets.ReturnValue);
      else
        Debug.Assert(false);
      return delegateNode;
    }
      ...
      if (getEnumerator != null) {
        ...
        if (tempScope.CapturedForClosure) {
          ...
          if (f.Type != SystemTypes.String) { // Should be any immutable type, but string is the only one so far!
            f.Attributes.Add(
              new AttributeNode(new MemberBinding(null, SystemTypes.OwnedAttribute.GetConstructor(SystemTypes.String)),
              new ExpressionList(new Literal("peer", SystemTypes.String)), AttributeTargets.Field));
          }
    ...
    public override Statement VisitExpose(Expose Expose){
      ...
      InvariantList justToForceDeserialization = exposeInstanceType.Contract.Invariants;
        ...
        if (declaredPointerType != null) {
          ...
          if (arrayAddressOfElem0 != null && arrayAddressOfElem0.Operand is Indexer) {
            ...
            if (arrayElem0.ArgumentListIsIncomplete) {
              ...
              TernaryExpression cond = new TernaryExpression(arrayIsNull, Literal.Null, arrayAddressOfElem0, NodeType.Conditional, t);
            ...
            StatementList statements = new StatementList(assignment, new ExpressionStatement(add));
            f.Initializer = new BlockExpression(new Block(statements), t);
            noPinning = true;
          }
        }
...
//      InterfaceList iface_ifaces = this.GetTypeView(iface).Interfaces;
    ...
    private System.Collections.Generic.Set<Expression> subExpsDependentOnQuantifiedVars;

    class DependentSubExpSeeker : StandardVisitor {
      System.Collections.Generic.Set<string> quantifiedVars;
      System.Collections.Generic.Stack<Expression> trace;
      System.Collections.Generic.Set<Expression> dependents;
      public System.Collections.Generic.Set<Expression> Dependents {
        get {
          return dependents;
        }
      }
      public DependentSubExpSeeker(System.Collections.Generic.Set<string> quantifiedVars) {
        this.quantifiedVars = quantifiedVars;
        trace = new System.Collections.Generic.Stack<Expression>();
        dependents = new System.Collections.Generic.Set<Expression>();
      }
      public override Expression VisitMemberBinding(MemberBinding memberBinding) {
        if (memberBinding != null && memberBinding.BoundMember != null) {
          if (this.quantifiedVars.Contains(memberBinding.BoundMember.Name.Name)) {
    ...
    class OldOperatorDistributor : StandardVisitor {
      public OldOperatorDistributor(System.Collections.Generic.Set<Expression> dependents) {
      ...
      public override Expression VisitMemberBinding(MemberBinding memberBinding) {
        if (memberBinding != null && (memberBinding.TargetObject is This || memberBinding.TargetObject is ImplicitThis)
          && dependents.Contains(memberBinding)) {
      ...
      public override Expression VisitIndexer(Indexer indexer) {
        ...
        foreach (Expression e1 in indexer.Operands) {
          if (this.dependents.Contains(e1)) {
            operandsDependentOnQuantifiedVar = true;
            break;
          }
        }
        bool objectDependentOnQuantifiedVar = this.dependents.Contains(indexer.Object);
    ...
    private void collectSubExpsDependentOnQuantifiedVars(Expression e) {
      this.subExpsDependentOnQuantifiedVars = new System.Collections.Generic.Set<Expression>();
      System.Collections.Generic.Set<string> boundNames = new System.Collections.Generic.Set<string>();
      foreach (ExpressionList exps in this.quantifiedVarStack) {
        foreach (Expression exp in exps) {
          ...
          if (mb == null) continue;
          boundNames.Add(mb.BoundMember.Name.Name);
        }
      }
    ...
    public override RequiresList VisitRequiresList(RequiresList Requires){
      ...
      bool addDefaultPrecondition = 
        ...
        && this.currentMethod.GetAttribute(SystemTypes.NoDefaultActivityAttribute) == null
        ...
        if (be != null && be.Block != null && be.Block.Statements != null) {
          ...
          if (n > 0) {
            ...
            if (s != null && s.Expression is Local) {
              s.Expression = new UnaryExpression(s.Expression, NodeType.AddressOf, s.Expression.Type.GetReferenceType());
              return be;
            }
          }
        } 
    ...
    public override Expression VisitTargetExpression(Expression target){
      ...
      switch(target.NodeType){
        ...
        case NodeType.MemberBinding: { 
          ...
          if (prop != null){
            if (prop.Setter == null) return null;
            bool baseCall = mb.TargetObject is Base;
            mb = new MemberBinding(mb.TargetObject, prop.Setter);
            ...
            if (!baseCall && prop.Setter.IsVirtualAndNotDeclaredInStruct) 
              call.NodeType = NodeType.Callvirt;
            call.Type = SystemTypes.Void;
            return call;
          }
