...
namespace System.Compiler.Analysis{
    ...
    public class Reentrancy : ConsistencyState /*PoinstToState */, IReentrancyInfo, IDataFlowState
    {
      ...
      public Reentrancy(PointsToState pts) : base(pts) {
        this.callEdges = new Edges();
        // this.ofendingCallees = new Set<Label>();
        // this.ofendingCallees = new Set<Pair<Label,int>>();
        this.ofendingCallees = new Dictionary<Label, int>();
        ...
        public override bool Includes(PointsToState cState2)
        {
            ...
            includes = includes && callEdges.Includes(cR.callEdges);
            return includes;
        }
        #endregion
      ...
      public override PointsToState CreateSummaryForFakeCallee(Variable vr, Method callee, Variable receiver, ExpressionList arguments, Label lb) {
        PointsToState pts = base.CreateSummaryForFakeCallee(vr, callee, receiver, arguments, lb);
        Reentrancy reentrancyState = new Reentrancy(pts);
        foreach (Parameter p in reentrancyState.Parameters) {
          Nodes pns = reentrancyState.Values(p);
          AddEdges(pns, pns, reentrancyState.CallEdges);
        }
        ...
        {
            // OJO ApplyNonAnalyzableCallReentrancyUsingEdges(callee, receiver, arguments, lb);
            ...
            if (!CciHelper.IsConstructor(callee))
            {
                if (!Method.IsStatic)
                {
                    ...
                    else
                    {
                        ...
                        if (thisVariableValues.IntersectionNotEmpty(allRecievers.Nodes))
                        {
                            // A call to this 
                            reentrantCall = true;
            ...
            Label lb, Receivers potentialReceivers, InterProcMapping ipm)
        {
            ...
            bool rule3b;
            Nodes withnesses;
            bool consistentReentrant;
            rule3b = CheckRule3UsingEdges(calleecState, ipm, out withnesses, out consistentReentrant);
            ...
            if (!CciHelper.IsConstructor(callee))
            {
                ...
                if (!Method.IsStatic)
                {
                    ...
                    bool rule1 = thisVariableValues.IntersectionNotEmpty(potentialReceivers.Nodes);
                    ...
                    if (rule1 || rule2)
                    {
                        ...
                        if (!IsExposable(offendingNodes)
                          ...
                          AddOffendingCall(lb, rule+10, offendingNodes);
                        }
            ...
            if (consistentReentrant && !thisCallIsReentrant) {
              AddOffendingCall(lb, 10+3, withnesses);
            }
        ...
        private static bool CheckRule2UsingEdges(Nodes bindedPotentialCalleeReceivers, Nodes thisVariableValues)
        {
            ...
            bool rule2 = thisVariableValues.IntersectionNotEmpty(bindedPotentialCalleeReceivers);
            return rule2;
        }
        ...
        private bool CheckRule3UsingEdges(Reentrancy calleeReentrancy, InterProcMapping ipm,
            out Nodes withnesses, out bool consistentReentrant)
        {
          consistentReentrant = false;
            bool res = false;
            ...
            foreach (Edge e in calleeReentrancy.callEdges)
            {
                ...
                bool rule3=false;
                // Nodes relatedReceiver = ipm.Related(ce.Src);
                // OJO!! if(!ce.ReceiverExposable)
                    rule3 = CheckLoopUsingCallEdgesAndIPM(e.Src, ipm, calleeReentrancy.callEdges, out withnesses, out consistentReentrant);
                res = res || rule3;
                if (res)
                    break;
            }
        ...
        public bool CheckLoopUsingCallEdgesAndIPM(IPTAnalysisNode n, InterProcMapping ipm,
            Edges cEdges, out Nodes withnesses, out bool consistentReentrant)
        {
            ...
            withnesses= DFSPathFrom(n, cEdges, visited, visitedMuApplied, ipm, out consistentReentrant);
        ...
        public Nodes DFSPathFrom(IPTAnalysisNode nr, Edges cEdges,
            ...
            InterProcMapping ipm, out bool consistentReentrant)
        {
          consistentReentrant = false;
            // Set<List<Edge>> paths = new Set<List<Edge>>();
            List<Edge> current = new List<Edge>();
            Nodes w = DFSPathFrom(nr, cEdges, visited, visitedMuApplied, ref current,ipm, ref consistentReentrant);
            return w;
        }
        ...
        public Nodes DFSPathFrom(IPTAnalysisNode nr, Edges cEdges,
            ...
            InterProcMapping ipm, ref bool consistentReentrant)
        {
            ...
            foreach (IPTAnalysisNode n in a)
            {
                ...
                if (adjacents.Count != 0)
                {
                    ...
                    foreach (Edge e in adjacents)
                    {
                        ...
                        // OJO BUG BUG: This may not be a loop. It can happen in a DAG.
                        // This is still sound but much more conservative
                        if (!visited.Contains(n2))
                        {
                            // if (visitedMuApplied.Intersection(relatedN2).Count == 0)
                            if (visitedMuApplied.IntersectionEmpty(relatedN2))
                            {
                                //hasLoop = DFSPathFrom(n2, cEdges, visited, visitedMuApplied, ipm);
                              bool consistentReentrant2 = consistentReentrant;              
                                withnesses.AddRange(DFSPathFrom(n2, cEdges, visited, visitedMuApplied, 
                                  ref newCurrent, ipm, ref consistentReentrant2));
                                consistentReentrant = consistentReentrant || consistentReentrant2;
                            }
                            else
                            {
