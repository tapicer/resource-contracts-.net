...
namespace System.Compiler{
  ...
  public class AssemblyNode : Module{ //An assembly is a module with a strong name
    ...
    internal void InitializePostAssemblyLoad (AssemblyNode referringModule) {
      if (referringModule != null && referringModule.AfterAssemblyLoad != null) {
        this.AfterAssemblyLoad = referringModule.AfterAssemblyLoad; // pass it along so any assembly references from "this" also get it
      }
    }
    public virtual void AfterAssemblyLoadProcessing () {
      if (this.AfterAssemblyLoad != null) {
        this.AfterAssemblyLoad(this);
      }
    }
#endif
  }
  ...
  public class EnsuresFinally : Ensures {
    public EnsuresFinally ()
      : base(NodeType.EnsuresFinally) {
    }
    public EnsuresFinally (Expression expression)
      : base(NodeType.EnsuresFinally, expression) {
    }
  }
  ...
  public class ModelfieldContract : Node {
      ...
      : base(NodeType.ModelfieldContract)
    {
      ...
      if (declaringType is Class) {
        ...
        if (cCtor != null)
          getM.Attributes.Add(new AttributeNode(new MemberBinding(null, cCtor), null, AttributeTargets.Method));
        ...
        for (int i = 0, j = 0; i < nrOfAttrs; i++) {          
          ...
          if (i == j && attrI != null && attrI.Name == "Rep") { //test i == j for slightly improved error handling on multiple [Rep]'s
            InstanceInitializer oCtor = SystemTypes.OwnedAttribute.GetConstructor();
            if (oCtor != null) { 
  ...
  public class Method : Member{
    ...
    public bool IsConfined {
      get{
        return this.GetAttributeFromSelfOrDeclaringMember(SystemTypes.ConfinedAttribute) != null ||
          (this.ApplyDefaultContract && this.IsPropertyGetter && !(this.DeclaringType is Struct) &&
           this.GetAttributeFromSelfOrDeclaringMember(SystemTypes.PureAttribute) == null &&
           this.GetAttributeFromSelfOrDeclaringMember(SystemTypes.StateIndependentAttribute) == null);
      }
    }
    ...
    public bool IsStateIndependent{
      get{
        return (this.ApplyDefaultContract && this.IsPropertyGetter && this.DeclaringType is Struct) ||
          this.GetAttributeFromSelfOrDeclaringMember(SystemTypes.StateIndependentAttribute) != null;
      }
    }
#endif
