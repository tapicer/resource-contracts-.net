  ...
  internal unsafe class Reader : IDisposable{
    ...
    private void AddMethodsToEvent(int eventIndex, Event/*!*/ evnt) {
      ...
      for (; i < n; i++){
        ...
        if (meth.Association == codedEventIndex){
          ...
          if (handlerFlags == (MethodFlags)0) handlerFlags |= eventMethod.Flags;
          switch(meth.Semantics){
            case 0x0008 : evnt.HandlerAdder = eventMethod; break;
            case 0x0010 : evnt.HandlerRemover = eventMethod; break;
    ...
    internal AssemblyNode/*!*/ GetAssemblyFromReference(AssemblyReference/*!*/ assemblyReference) {
      ...
      cacheIt:
        ...
        if (referringAssembly != null) {
          assembly.InitializePostAssemblyLoad(referringAssembly);
        }
