...
namespace Microsoft.Contracts{
  ...
  public class Contract
  {
    ...
    /// WritableBytes(ptr) >= ..., the runtime assetions will not fail.
    /// </summary>
    const ulong MaxWritableExtent = ulong.MaxValue;

    /// <summary>
    /// Allows specifying a writable extent for a UIntPtr, similar to SALs writable extent.
    /// NOTE: this is for static checking only. No useful runtime code can be generated for this
    /// at the moment.
    /// </summary>
    /// <param name="ptr">Start of memory region</param>
    /// <returns>The result is the number of bytes writable starting at ptr</returns>
    [Pure]
    public static ulong WritableBytes(UIntPtr ptr) { return MaxWritableExtent; }
    ...
    /// <returns>The result is the number of bytes writable starting at ptr</returns>
    [Pure]
    public static ulong WritableBytes(IntPtr ptr) { return MaxWritableExtent; }
    ...
    /// <returns>The result is the number of bytes writable starting at ptr</returns>
    [Pure]
    unsafe public static ulong WritableBytes(void* ptr) { return MaxWritableExtent; }
  ...
  */
  ...
  /// <summary>
  /// I added an argument to Pure to specified if we want to assumed purity. That means to force the analysis 
  /// to avoid checking for that method but still believe the method is pure (Herman wanted this feature)
  /// </summary>
    [AttributeUsage(Microsoft.Contracts.SpecTargets.Code | AttributeTargets.Parameter, AllowMultiple = false, Inherited = true)]
...
namespace Microsoft.Contracts{
  ...
  public sealed class Owner{
    ...
    /// The following methods perform the corresponding operations on the
    /// element owner of an array.
    [Pure]
    public static bool ElementSame([NotNull] [Delayed] object[] c, [NotNull] object d) { return true; }
    [Pure]
    public static bool ElementDifferent([NotNull] [Delayed] object[] c, [NotNull] object d) { return true; }
    [Pure]
    public static bool ElementNone([NotNull] [Delayed] object[] c) { return true; }
    [Pure]
    public static bool ElementIs([NotNull] [Delayed] object[] subject, [NotNull] object owner, [NotNull] Type frame) { return true; }
    [Pure]
    public static bool ElementNew([NotNull] [Delayed] object[] c) { return true; }
    public static void ElementAssign([NotNull] [Delayed] object[] subject, [NotNull] object owner, [NotNull] Type frame) { }
    public static void ElementAssignSame([NotNull] [Delayed] object[] subject, [NotNull] object peer) { }
  }
